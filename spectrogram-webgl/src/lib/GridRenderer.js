/**
 * Renders vertical lines along X axis on Y=0, Z=0 plane using WebGL
 */
...
export class GridRenderer {
  constructor(gl, canvas) {
    ...
    this.gridStep = 0.64;
    this.visible = false;
    this.lineHeight = 5; // Vertical extent of each line
    this.xCoordinates = []; // Store X coordinates for labeling
  ...
  initGeometry() {
    ...
    // Generate vertical lines along X axis at Y=0, Z=0
    // Lines go from (x, 0, 0) upward and downward

    this.xCoordinates = []; // Reset coordinates array

    for (let x = this.gridMin; x <= this.gridMax; x += this.gridStep) {
      const isMajor = Math.abs(x % (this.gridStep * 10)) < 0.0001; // Every 10 steps
      const color = isMajor ? majorColor : gridColor;

      // Vertical line from (x, -lineHeight, 0) to (x, lineHeight, 0)
      lines.push(x, -this.lineHeight, 0, ...color);
      lines.push(x, this.lineHeight, 0, ...color);
  ...
  drawLabels(mvpMatrix) {
    ...
    // Draw coordinate labels for each X line
    this.xCoordinates.forEach(x => {
      // Project the midpoint of the line to 2D screen space
      const screenPos = this.projectPoint([x, 0, 0], mvpMatrix);
