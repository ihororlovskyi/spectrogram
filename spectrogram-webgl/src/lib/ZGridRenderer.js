/**
 * Renders lines along Z axis on X=0, Y=0 plane using WebGL
 */

export class ZGridRenderer {
  constructor(gl, canvas) {
    ...
    this.gridStep = 0.64;
    this.visible = false;
    this.lineDepth = 5; // Depth extent of each line
    this.zCoordinates = []; // Store Z coordinates for labeling
  ...
  initGeometry() {
    ...
    // Generate lines along Z axis at X=0, Y=0
    // Lines go from (0, 0, z) forward and backward

    this.zCoordinates = []; // Reset coordinates array

    for (let z = this.gridMin; z <= this.gridMax; z += this.gridStep) {
      const isMajor = Math.abs(z % (this.gridStep * 10)) < 0.0001; // Every 10 steps
      const color = isMajor ? majorColor : gridColor;

      // Line from (0, -lineDepth, z) to (0, lineDepth, z)
      lines.push(0, -this.lineDepth, z, ...color);
      lines.push(0, this.lineDepth, z, ...color);

      // Store coordinate for labeling
      this.zCoordinates.push(z);
    }
  ...
  drawLabels(mvpMatrix) {
    ...
    // Draw coordinate labels for each Z line
    this.zCoordinates.forEach(z => {
      // Project the midpoint of the line to 2D screen space
      const screenPos = this.projectPoint([0, 0, z], mvpMatrix);
